...
public class AltaGrupoFamiliarJPanel extends JPanel {
	...
	public AltaGrupoFamiliarJPanel(MenuJFrame frame, Alumno alumno) {
		...
		tableHermano.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Nombre", "Apellido", "Edad", "Ocupacion", "Escuela", "Convive" }));
		...
		tableEnfermedadCronica
				.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "Diagnóstico", "Gasto mensual" }));
		...
		btnAgregarEnfermedadCronica.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				AltaEnfermedadCronicaJFrame altaEnfermedadCronicaJFrame = new AltaEnfermedadCronicaJFrame(
						altaGrupoFamiliarJPanel, grupoFamiliarNuevo);
		...
		btnAgregarHermano.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				AltaHermanoJFrame altaHermanoJFrame = new AltaHermanoJFrame(altaGrupoFamiliarJPanel,
						grupoFamiliarNuevo);
	...
	public void actualizarTablaProgenitor(GrupoFamiliar grupoFamiliar) {
		...
		ProgenitorTutor unPro = listaProgenitorTutor.get(listaProgenitorTutor.size() - 1);

		Object row[] = { unPro.getNombre(), unPro.getApellido(), unPro.getIngresoNeto(), unPro.getEdad(),
				unPro.getOcupacion(), unPro.getLugarDeTrabajo(), ((unPro.getConvive())?"Si":"No") };
	...
	public void actualizarTablaHermano(GrupoFamiliar grupoFamiliar) {
		...
		Hermano unHer = listaHermanos.get(listaHermanos.size() - 1);

		Object row[] = { unHer.getNombre(), unHer.getApellido(), unHer.getEdad(), unHer.getOcupacion(),
				unHer.getEscuela(),((unHer.getConvive())?"Si":"No") };
	...
	public void actualizarTablaEnfermedadCronica(GrupoFamiliar grupoFamiliar) {
		...
		EnfermedadCronica unaEnf = listaEnfermedadCronica.get(listaEnfermedadCronica.size() - 1);
